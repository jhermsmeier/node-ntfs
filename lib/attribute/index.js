...
/**
 * Create a new NTFS Attribtue Record
 * @param  {Object} options
 * @return {Attribute}
 */
Attribute.create = function( options ) {
  return new Attribute( options )
}

/**
 * Parse a NTFS Attribute Record
 * @param  {Buffer} buffer
 * @return {Attribute}
 */
Attribute.parse = function( buffer ) {
  return new Attribute().parse( buffer )
}
...
Attribute.prototype = {
  ...
  parse: function( buffer ) {
    ...
    if( this.nameLength > 0 ) {
      this.name = buffer.toString( 'ucs2',
        this.nameOffset,
    ...
    if( this.isResident ) {
      ...
      this.lowVCN             = int64.readUIntLE( buffer, 16 )
      // Highest valid Virtual Cluster Number
      this.highVCN            = int64.readUIntLE( buffer, 24 )
      this.mappingPairsOffset = buffer.readUInt16LE( 32 )
      this.compressionUnit    = buffer.readUInt8( 34 )
      // 5 bytes reserved / alignment
      this.allocatedSize      = int64.readIntLE( buffer, 40 )
      this.dataSize           = int64.readIntLE( buffer, 48 )
      this.initializedSize    = int64.readIntLE( buffer, 56 )
      this.compressedSize     = int64.readIntLE( buffer, 64 )
