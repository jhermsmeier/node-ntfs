 ...
 * @type {Function}
 */
Attribute.Definition = require( './attribute-definition' )
...
Attribute.prototype = {
  ...
  parse: function( buffer ) {
    ...
    if( this.nonResident === 0 ) {
      ...
      this.lowVCN             = readUInt64LE( buffer, 16 )
      // Highest valid Virtual Cluster Number
      this.highVCN            = readUInt64LE( buffer, 24 )
      this.mappingPairsOffset = buffer.readUInt16LE( 32 )
      this.compressionUnit    = buffer.readUInt8( 34 )
      // 5 bytes reserved / alignment
      this.allocatedSize      = readInt64LE( buffer, 40 )
      this.dataSize           = readInt64LE( buffer, 48 )
      this.initializedSize    = readInt64LE( buffer, 56 )
      this.compressedSize     = readInt64LE( buffer, 64 )
