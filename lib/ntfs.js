...
/**
 * Runs a check (chkdsk) against a given volume
 * @param {NTFS.Volume} volume
 * @param {Object} [options]
 * @param {Function} [callback]
 * @return {EventEmitter}
 */
NTFS.check = function( volume, options, callback ) {
  ...
  var check = new NTFS.CheckDisk( volume, options )
  
  if( typeof callback === 'function' ) {
    check.once( 'error', callback )
    check.once( 'end', callback )
  }
